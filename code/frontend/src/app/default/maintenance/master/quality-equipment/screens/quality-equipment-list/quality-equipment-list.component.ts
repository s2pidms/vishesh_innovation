...
import {
    QUALITY_EQUIPMENT_MASTER_REPORT_DATA,
    QUALITY_EQUIPMENT_MASTER_PDF_DATA
} from "@mocks/export-data/maintenance/master";
import {QualityEquipment} from "@mocks/models/maintenance/masters";
import {ConfirmDeleteComponent} from "@shared/modals";
import {NgbModal} from "@ng-bootstrap/ng-bootstrap";

@Component({
    selector: "app-quality-equipment-list",
    templateUrl: "./quality-equipment-list.component.html"
})
export class QualityEquipmentListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.qualityEquipmentService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    delete(id: any) {
        this.spinner.show();
        this.qualityEquipmentService.delete(id).subscribe(success => {
            this.spinner.hide();
    ...
    openConfirmModal(id: any, code: any) {
        ...
        modalRef.componentInstance.confirmText = `Confirm Deletion of Equipment Code ${code} ?`;
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(QUALITY_EQUIPMENT_MASTER_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = QUALITY_EQUIPMENT_MASTER_PDF_DATA(data);
