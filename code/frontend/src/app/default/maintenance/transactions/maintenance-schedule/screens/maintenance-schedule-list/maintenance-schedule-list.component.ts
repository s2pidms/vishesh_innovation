...
import {
    MAINTENANCE_SCHEDULING_REPORT_DATA,
    MAINTENANCE_SCHEDULING_PDF_DATA
} from "@mocks/export-data/maintenance/transactions";
import {MaintenanceSchedule} from "@mocks/models/maintenance/transactions";
...
export class MaintenanceScheduleListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.maintenanceScheduleCreationService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(MAINTENANCE_SCHEDULING_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = MAINTENANCE_SCHEDULING_PDF_DATA(data);
