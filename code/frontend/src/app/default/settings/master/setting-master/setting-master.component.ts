...
import {AppGlobalService, StorageService, MenuTitleService, SpinnerService} from "@core/services";
...
export class SettingMasterComponent implements OnInit {
    constructor(
    ...
    getAll() {
        ...
        this.subModulesService.getAll(payload).subscribe(success => {
            this.cards = success?.rows.find((x: any) => x.title == "Setting Master");
