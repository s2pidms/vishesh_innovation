...
import {ModuleMasterService} from "@services/settings";
...
@Component({
    selector: "app-module-master-form",
    templateUrl: "./module-master-form.component.html"
})
export class ModuleMasterFormComponent implements OnInit {
    submitted = false;
    ...
    form = new UntypedFormGroup({
        ...
        parameterLabel: new UntypedFormControl(null),
        parameterName: new UntypedFormControl(null),
    ...
    create(formData: any) {
        this.spinner.show();
        this.moduleMasterService.create(formData).subscribe(success => {
            this.submitted = false;
            this.spinner.hide();
            this.toastService.success(success.message);
            this.router.navigate([`/default/settings/master/module_master/list/${this.type}`]);
        });
    }

    update(formData: any) {
        this.spinner.show();
        this.moduleMasterService.update(formData._id, formData).subscribe(success => {
            this.spinner.hide();
            this.submitted = false;
            this.toastService.success(success.message);
            this.router.navigate([`/default/settings/master/module_master/list/${this.type}`]);
        ...
        this.activatedRoute.queryParams
            .pipe(
                mergeMap((params: any) => {
                    ...
                    if (params["id"]) {
                        return this.moduleMasterService.getById(params["id"]);
                    } else {
                        return of({});
                    }
                })
