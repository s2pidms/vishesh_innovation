...
import {FGIE_FORM_ERRORS} from "@mocks/validations/stores";
import {LIST_DEFAULT_PERMISSION_ACTIONS} from "@mocks/constant";
import {IFGIEMasterData} from "@mocks/models/stores/transactions";
...
export class FormComponent implements OnInit {
    ...
    getInitialData() {
        ...
        this.finishedGoodsInwardEntryServices.getAllFGINMasterData({}).subscribe(result => {
            this.masterData = result;
            this.form.controls["FGINNo"].setValue(this.masterData.autoIncrementNo);
    ...
    submit() {
        this.submitted = true;
        if (this.validationService.checkErrors(this.form, FGIE_FORM_ERRORS)) {
            return;
        }
    ...
    addFGINInfo() {
        ...
        let SKUNUM = this.masterData?.SKUMastersOptions?.find((x: any) => x.value == this.form.controls["SKUId"].value)?.skuNum ?? "";

        let SKUNAME = this.masterData?.SKUMastersOptions?.find((x: any) => x.value == this.form.controls["SKUId"].value)?.label ?? "";
