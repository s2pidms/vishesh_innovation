...
@Component({
    selector: "app-sps-duc-modal",
    templateUrl: "./sps-duc-modal.component.html"
})
export class SpsDucModalComponent implements OnInit {
    @Input() action: any = "";
    ...
    form: any = new UntypedFormGroup({
        ...
        U1Qty: new UntypedFormControl(1),
        U2Qty: new UntypedFormControl(1),
    ...
    setDescription() {
        ...
        if (width && length) {
            let primaryToSecondaryConversion = +((width * length) / DF).toFixed(2);
            this.f["primaryToSecondaryConversion"].setValue(+primaryToSecondaryConversion);

            let itemDescription = `${width} ${widthUnit} X ${length} ${lengthUnitValue}`;
            this.f["itemDescription"].setValue(itemDescription);
        } else {
            // this.f["primaryToSecondaryConversion"].setValue(0);
            this.f["itemDescription"].setValue(null);
        }

        this.f["primaryToSecondaryConversion"].value;

        this.f["U2Qty"].setValue(+this.f["primaryToSecondaryConversion"].value);
        this.f["U1Qty"].setValue(1);
        // this.setU1Qty();
        // this.setU2Qty();
    }

    setU1Qty() {
        if (!this.f["primaryToSecondaryConversion"].value) {
            this.toastService.warning("Conversion of Units is required");
            return;
        }

        let U1Qty = this.f["U1Qty"].value;
        let U2Qty = this.f["U2Qty"].value;

        let quantity = +(+U2Qty / +this.f["primaryToSecondaryConversion"].value).toFixed(2);
        this.f["U1Qty"].setValue(quantity);
    }
    setU2Qty() {
        if (!this.f["primaryToSecondaryConversion"].value) {
            this.toastService.warning("Conversion of Units is required");
        ...
        let quantity = +(+this.f["primaryToSecondaryConversion"].value * +U1Qty).toFixed(2);
        this.f["U2Qty"].setValue(quantity);
    }
    setLengthData() {
        let length = +this.f["length"].value;
