...
@Component({
    selector: "app-roll-to-sheet-form",
    templateUrl: "./roll-to-sheet-form.component.html"
})
export class RollToSheetFormComponent implements OnInit {
    ...
    setSheetDetails(item: any, index: number) {
        ...
        this.balanceDetails = arr.map((r: any, index: number) => {
            ...
            return {
                ...
                item: element?.item,
                width: element?.width,
                closedIRQty: index === 0 ? r * SQM : remainingPPICQty,
                length: index === 0 ? element.length : +remainingLength.toFixed(2),
                SQM: index === 0 ? element.SQM : +remainingSQM.toFixed(2),
                roll: index == 0 ? r : 1,
                type: "SFGStock",
