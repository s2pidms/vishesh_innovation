...
import {BatchInfoModalComponent, JobCardMRPModalComponent} from "../../../job-card/screens/components";

@Component({
    selector: "app-job-card-planning-form",
    templateUrl: "./job-card-planning-form.component.html",
    styleUrls: ["./job-card-planning-form.component.scss"]
})
export class JobCardPlanningFormComponent implements OnInit {
        ...
        this.activatedRoute.queryParams
            .pipe(
                mergeMap((params: any) => {
                    ...
                    if (params["SKUId"] || params["SOId"]) {
                        if (params["jobCardId"]) {
                            return this.jobCardCreationService.getById(params["jobCardId"]);
                        } else {
                            return this.jobCardCreationService.getAllJCTableList({
                                SOId: params["SOId"],
                                customerId: params.customerId,
                                SKUId: params.SKUId,
                                creationFlag: params.creationFlag
                            });
                        }
                    } else {
                        return of({});
            ...
            .subscribe((success: any) => {
                ...
                if (success?.masterData) {
                    this.SKUDetailsOfJC = success?.rows;
                    this.collection = this.SKUDetailsOfJC?.length;
                    this.customerInfo = success?.masterData;
                    if (this.customerInfo) {
                        this.f["stage"].setValue(this.customerInfo?.stage);
                        if (this.customerInfo.jobCardNo) {
                            this.f["jobCardNo"].setValue(this.customerInfo?.jobCardNo);
                            this.f["jobCardDate"].setValue(
                                this.utilityService.getFormatDate(this.customerInfo?.jobCardDate, "YYYY-MM-DD")
                            );
                            this.f["_id"].setValue(this.customerInfo?.jobCard);
                            this.f["batchInfo"].patchValue(this.customerInfo?.batchInfo);
                            this.batchInfo.controls["manufacturingDate"].setValue(
                                this.utilityService.getFormatDate(
                                    this.customerInfo?.batchInfo?.manufacturingDate,
                                    "YYYY-MM-DD"
                                )
                            );
                        } else {
                            this.f["jobCardNo"].setValue(success?.autoIncrementNo);
                            this.f["jobCardDate"].setValue(this.utilityService.getTodayDate("YYYY-MM-DD"));
                        }
                        this.f["customerName"].setValue(this.customerInfo?.customerName);
                        this.f["customerCategory"].setValue(this.customerInfo?.customerCategory);
                        this.f["reference"].setValue(this.customerInfo?.reference);
                        this.f["referenceModel"].setValue(this.customerInfo?.referenceModel);
                        this.f["orderType"].setValue(this.customerInfo?.orderType);
                    }
                } else {
    ...
    openBatchInfoModal() {
        ...
        modalRef.result.then(
            (success: any) => {
                if (success && ["create", "edit", "Create JC"].includes(this.action)) {
