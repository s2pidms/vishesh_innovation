...
import {BOM_OF_CHILD_REPORT_DATA, BOM_OF_CHILD_PDF_DATA} from "@mocks/export-data/planning/master";
import {BOMOfChildPart} from "@mocks/models/planning/masters";
import {ConfirmDeleteComponent} from "@shared/modals";
import {NgbModal} from "@ng-bootstrap/ng-bootstrap";

@Component({
    selector: "app-bom-child-item-list",
    templateUrl: "./bom-child-item-list.component.html"
})
export class BomChildItemListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.bomChildPartService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(BOM_OF_CHILD_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = BOM_OF_CHILD_PDF_DATA(data);
    ...
    delete(id: any) {
        this.spinner.show();
        this.bomChildPartService.delete(id).subscribe(success => {
            this.spinner.hide();
