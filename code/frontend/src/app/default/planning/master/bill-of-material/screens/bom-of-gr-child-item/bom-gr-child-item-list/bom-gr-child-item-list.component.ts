...
import {
    BOM_OF_GRAND_CHILD_PART_REPORT_DATA,
    BOM_OF_GRAND_CHILD_PART_PDF_DATA
} from "@mocks/export-data/planning/master";
import {BOMOfGrChildPart} from "@mocks/models/planning/masters";
import {ConfirmDeleteComponent} from "@shared/modals";
import {NgbModal} from "@ng-bootstrap/ng-bootstrap";
@Component({
    selector: "app-bom-gr-child-item-list",
    templateUrl: "./bom-gr-child-item-list.component.html"
})
export class BomGrChildItemListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.bomGrandChildItemService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(BOM_OF_GRAND_CHILD_PART_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = BOM_OF_GRAND_CHILD_PART_PDF_DATA(data);
    ...
    delete(id: any) {
        this.spinner.show();
        this.bomGrandChildItemService.delete(id).subscribe(success => {
            this.spinner.hide();
