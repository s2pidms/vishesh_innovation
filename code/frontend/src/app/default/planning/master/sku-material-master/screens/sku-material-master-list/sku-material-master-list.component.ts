...
import {CopyMaterialMasterModalComponent} from "../components/copy-material-master-modal/copy-material-master-modal.component";
...
@Component({
    selector: "app-sku-material-master-list",
    templateUrl: "./sku-material-master-list.component.html"
})
export class SkuMaterialMasterListComponent implements OnInit, OnDestroy {
    ...
    navigateTo(path: string, item: any, action: string) {
        if (
            (item.SKUAttributesStatus == "Inactive" && action == "material") ||
    ...
    openCopyMaterialMasterModal(item: any) {
        if (item.SKUAttributesStatus == "Active") {
            const modalRef = this.modalService.open(CopyMaterialMasterModalComponent, {
                centered: true,
                size: "xl",
                backdrop: "static",
                keyboard: false
