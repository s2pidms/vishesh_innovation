...
import {PROCESS_MASTER_PDF_DATA, PROCESS_MASTER_REPORT_DATA} from "@mocks/export-data/planning/master";
import {ConfirmDeleteComponent} from "@shared/modals";
import {NgbModal} from "@ng-bootstrap/ng-bootstrap";
@Component({
    selector: "app-process-master-list",
    templateUrl: "./process-master-list.component.html"
})
export class ProcessMasterListComponent implements OnInit, OnDestroy {
    @ViewChildren(NgbdSortableHeader) headers: QueryList<NgbdSortableHeader> | any;

    page: number = 1;
    pageSize: number = 8;
    collection: number = 0;
    column: string = "processId";
    direction: number = -1;
    search: string = "";
    tableData: ProcessMaster[] = [];
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.processMasterService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    pdfDownload(data: any) {
        let outPut = PROCESS_MASTER_PDF_DATA(data);
        this.exportToPDFService.generatePdf(outPut.tableData, outPut.title);
    }
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(PROCESS_MASTER_REPORT_DATA(data));
    }
    ...
    delete(id: any) {
        this.spinner.show();
        this.processMasterService.delete(id).subscribe(success => {
            this.spinner.hide();
    ...
    openConfirmModal(id: any, code: any) {
        ...
        modalRef.componentInstance.confirmText = `Confirm Deletion of Process ID ${code} ?`;
