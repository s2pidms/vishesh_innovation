...
import {SKUProductionService} from "@services/production";
import {IProductionOfSKUMasterData, ISKUProductionDetailsArray} from "@mocks/models/production/transactions";

@Component({
    selector: "app-sku-production-form",
    templateUrl: "./sku-production-form.component.html"
})
export class SkuProductionFormComponent implements OnInit {
    ...
    masterData: IProductionOfSKUMasterData = {
        autoIncrementNo: "",
    ...
    constructor(
        private skuProductionService: SKUProductionService,
    ...
    form = new UntypedFormGroup({
        ...
        SKUPartProductionDetails: new UntypedFormControl([]),
    ...
    reset() {
        this.form.reset();
        this.masterData.SKUOptions = [];
        this.machineName = [];
        this.collection = this.masterData?.SKUOptions.length;
    ...
    submit() {
        ...
        formData.SKUPartProductionDetails = this.masterData?.SKUOptions;
        if (formData._id) {
    ...
    create(formData: any) {
        this.spinner.show();
        this.skuProductionService.create(formData).subscribe(success => {
            this.submitted = false;
            this.spinner.hide();
            this.toastService.success(success.message);
            this.router.navigate(["/default/production/transactions/child_part_production/sku_production/list"]);
        });
    }

    update(formData: any) {
        this.spinner.show();
        this.skuProductionService.update(formData._id, formData).subscribe(success => {
            this.spinner.hide();
            this.submitted = false;
            this.toastService.success(success.message);
            this.router.navigate(["/default/production/transactions/child_part_production/sku_production/list"]);
    ...
    getInitialData() {
        this.spinner.show();
        this.skuProductionService.getAllMasterData({}).subscribe(result => {
            ...
            this.activatedRoute.queryParams
                .pipe(
                    mergeMap((params: any) => {
                        ...
                        if (params["id"]) {
                            // get patch data
                            return this.skuProductionService.getById(params["id"]);
                        } else {
                            return of({});
                        }
                    })
                ...
                .subscribe((success: any) => {
                    ...
                    this.masterData.SKUOptions = success.SKUPartProductionDetails;
    ...
    changeRejQty(ele: any, i: any) {
        let index = this.masterData?.SKUOptions.map((x: any) => x.SKUCode).indexOf(ele?.SKUCode);
        if (ele.rejectedQty > ele.batchQty) {
            this.masterData.SKUOptions[index].rejectedQty = 0;
        }
        this.masterData.SKUOptions[index].outputQty = ele.batchQty - ele.rejectedQty;
    }

    changeOutPutQty(ele: any, i: any) {
        let index = this.masterData?.SKUOptions.map((x: any) => x.SKUCode).indexOf(ele?.SKUCode);
        if (ele.outputQty > ele.batchQty) {
            this.masterData.SKUOptions[index].outputQty = 0;
        }
        this.masterData.SKUOptions[index].rejectedQty = ele.batchQty - ele.outputQty;
    }

    setRejQty(eve: any, ele: any) {
        let index = this.masterData?.SKUOptions.map((x: any) => x.SKUCode).indexOf(ele?.SKUCode);
        this.masterData.SKUOptions[index].rejectedQty = Math.abs(eve);
    }
    setOutPutQty(eve: any, ele: any) {
        let index = this.masterData?.SKUOptions.map((x: any) => x.SKUCode).indexOf(ele?.SKUCode);
        this.masterData.SKUOptions[index].outputQty = Math.abs(eve);
    }
    ...
    preview() {
        this.search = "";
        this.ESCPreviewArr = this.masterData?.SKUOptions;
        this.masterData.SKUOptions = this.masterData?.SKUOptions.filter((x: any) => x.batchQty > 0);
        if (this.masterData.SKUOptions.length > 0) {
            this.isPreview = true;
    ...
    ESCPreview() {
        this.search = "";
        this.isPreview = false;
        this.masterData.SKUOptions = this.ESCPreviewArr;
        this.collection = this.masterData?.SKUOptions.length;
    }
    ...
    onSort({column, direction}: SortEvent) {
        ...
        if (direction === "" || column === "") {
            this.masterData.SKUOptions = this.masterData?.SKUOptions;
        } else {
            this.masterData.SKUOptions = [...this.masterData?.SKUOptions].sort((a: any, b: any) => {
