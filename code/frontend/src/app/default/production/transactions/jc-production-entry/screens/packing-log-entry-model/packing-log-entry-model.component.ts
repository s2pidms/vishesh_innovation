...
import {PackingLogService} from "@services/production/packingLog.service";

@Component({
    selector: "app-packing-log-entry-model",
    templateUrl: "./packing-log-entry-model.component.html"
})
export class PackingLogEntryModelComponent implements OnInit {
    ...
    create(formData: any) {
        this.spinner.show();
        this.packingLogService.createOrUpdate(formData).subscribe(success => {
            this.spinner.hide();
    ...
    getInitialData() {
        // get master data
        this.spinner.show();
        this.packingLogService.getAllMasterData(this.selectedDetails).subscribe(result => {
            if (result.packingLog) {
                if (result?.packingLog?.logEntry?.prodDate) {
                    result.packingLog.logEntry.prodDate = this.utilityService.getFormatDate(
                        result?.packingLog?.logEntry?.prodDate,
                        "YYYY-MM-DD"
                    );
                }
                this.form.patchValue(result.packingLog);
            }
