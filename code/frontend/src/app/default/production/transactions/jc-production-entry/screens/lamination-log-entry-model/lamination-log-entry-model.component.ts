...
import {LaminationLogService} from "@services/production/laminationLog.service";

@Component({
    selector: "app-lamination-log-entry-model",
    templateUrl: "./lamination-log-entry-model.component.html"
})
export class LaminationLogEntryModelComponent implements OnInit {
    ...
    constructor(
        ...
        private laminationLogService: LaminationLogService,
    ...
    create(formData: any) {
        this.spinner.show();
        this.laminationLogService.createOrUpdate(formData).subscribe(success => {
            this.spinner.hide();
    ...
    getInitialData() {
        // get master data
        this.spinner.show();
        this.laminationLogService.getAllMasterData(this.selectedDetails).subscribe(result => {
            if (result.laminationLog) {
                if (result?.laminationLog?.logEntry?.prodDate) {
                    result.laminationLog.logEntry.prodDate = this.utilityService.getFormatDate(
                        result?.laminationLog?.logEntry?.prodDate,
                        "YYYY-MM-DD"
                    );
                }
                this.form.patchValue(result.laminationLog);
            }
