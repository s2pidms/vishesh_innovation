...
import {
    FG_INVENTORY_REPORT_LOCATION_WISE_PDF_DATA,
    FG_INVENTORY_REPORT_LOCATION_WISE_REPORT_DATA
} from "@mocks/export-data/planning/reports";
import {FGIEReportLocationWise} from "@mocks/models/production/reports";

@Component({
    selector: "app-fgie-report-location-wise",
    templateUrl: "./fgie-report-location-wise.component.html"
})
export class FGIEReportLocationWiseComponent implements OnInit, OnDestroy {
    ...
    direction: number = -1;
    search: string = "";
    tableData: FGIEReportLocationWise[] = [];
    location: any = "";
    locations: any = [];
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.finishedGoodsInwardEntryService
            .getAllFGINLocationWiseReports(payload)
            .subscribe(success => {
                if (flag == "EXCEL") {
    ...
    pdfDownload(data: any) {
        let output = FG_INVENTORY_REPORT_LOCATION_WISE_PDF_DATA(data);
        this.exportToPDFService.generatePdf(output.tableData, output.title);
    }
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(FG_INVENTORY_REPORT_LOCATION_WISE_REPORT_DATA(data));
    }
