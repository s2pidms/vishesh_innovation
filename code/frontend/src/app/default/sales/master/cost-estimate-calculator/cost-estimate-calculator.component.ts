...
import {ICostEstimateCalculatorMasterData} from "@mocks/models/sales/master/costEstimateMasterData";
@Component({
...
export class CostEstimateCalculatorComponent implements OnInit {
    ...
    constructor(
        private SKUMasterService: SKUService,
    ...
    getAllMasterData() {
        this.spinner.show();
        this.costEstimateService.getAllSKUCostEstimateMasterData({}).subscribe(result => {
            if (result) {
    ...
    getById(ev: any) {
        ...
        this.SKUMasterService.getAllSKUMasterBySKUId(this.skuId).subscribe(success => {
                    ...
                    x.costPerSKUUnit = +Number(
                        this.masterData?.costSheetList[
                            this.masterData?.costSheetList.findIndex(
                                (y: any) => y.componentType == "Direct" && y.isTotal
                            )
                        ].costPerSKUUnit +
                            this.masterData?.costSheetList[
                                this.masterData?.costSheetList.findIndex(
                                    (y: any) => y.componentType == "Indirect" && y.isTotal
                                )
                            ].costPerSKUUnit
                    ).toFixed(2);
                }
