...
export class TransactionsTabsComponent implements OnInit {
    ...
    getAppParameterByCode() {
        this.spinner.show();
        this.appParameterService.getAppParameterValueByCode("ENABLE_DIRECT_TAX_INVOICE").subscribe(success => {
            if (success.appParameterValue != "Yes") {
                for (let i = 0; i < this.cards.length; i++) {
                    if (this.cards[i].title == "Direct Tax Invoice") {
