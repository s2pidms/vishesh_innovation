...
import {QUOTATION_SKU_REPORT_DATA, QUOTATION_SKU_PDF_DATA} from "@mocks/export-data/sales/reports";
import {
    AppGlobalService,
    ExportExcelService,
    ExportToPDFService,
...
export class QuotationSkuReportComponent implements OnInit, OnDestroy {
    ...
    navigateTo(u: any, action: string) {
        ...
        window.open(
            `${window.location.origin}/#/print/quotation_of_sku?id=${u?._id}&action=${action}&buttonCondition=${buttonCondition}`,
            "_blank"
        );
    }
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.quotationOfSKUService.getAllReports(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    getById(id: string) {
        this.spinner.show();
        this.quotationOfSKUService.getById(id).subscribe(success => {
            this.tableData = success.rows;
            this.originTableData = success.rows;
            this.collection = success.count;
            this.spinner.hide();
