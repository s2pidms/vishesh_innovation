...
import {PROFORMA_INVOICE_PDF_DATA, PROFORMA_INVOICE_REPORT_DATA} from "@mocks/export-data/sales/reports";
import {proformaInvoice} from "@mocks/models/sales/reports";
...
export class ProformaInvoiceReportComponent implements OnInit, OnDestroy {
    ...
    navigateTo(u: any, action: string) {
        ...
        window.open(
            `${window.location.origin}/#/print/pi_print?id=${u?._id}&action=${action}&buttonCondition=${buttonCondition}`,
            "_blank"
        );
    }
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.proformaInvoiceService.getAllReport(payload).subscribe(success => {
            if (flag == "EXCEL") {
                ...
                this.totalAmount = success?.totalAmounts?.PITotalAmount
                    ? (success?.totalAmounts?.PITotalAmount).toFixed(2)
                    : 0;
            }
            this.spinner.hide();
        });
    ...
    getById(id: string) {
        this.spinner.show();
        this.proformaInvoiceService.getById(id).subscribe(success => {
            this.tableData = success.rows;
            this.originTableData = success.rows;
            this.collection = success.count;
            this.spinner.hide();
