...
import {SaleDebitNote} from "@mocks/models/sales/reports";
import {SALES_DEBIT_NOTE_PDF_DATA, SALES_DEBIT_NOTE_REPORT_DATA} from "@mocks/export-data/sales/reports";
import {SaleDebitNoteService} from "@services/sales";

@Component({
    selector: "app-sales-debit-note-report",
    templateUrl: "./sales-debit-note-report.component.html"
})
export class SalesDebitNoteReportComponent implements OnInit, OnDestroy {
    ...
    navigateTo(id: any, action: string) {
        ...
        window.open(
            `${window.location.origin}/#/print/sale_debit_note?id=${id}&action=${action}&buttonCondition=${buttonCondition}`,
            "_blank"
        );
    }
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.saleDebitNoteService.getAllReports(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    pdfDownload(data: any) {
        let output = SALES_DEBIT_NOTE_PDF_DATA(data);
        this.exportToPDFService.generatePdf(output.tableData, output.title);
    }

    excelDownload(data: any) {
        this.exportExcelService.exportExcel(SALES_DEBIT_NOTE_REPORT_DATA(data));
    }
