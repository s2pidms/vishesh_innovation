...
import {EWayBillsTransportationDetailsComponent} from "../../../generate-e-way-bills/screens/e-way-bills-transportation-details/e-way-bills-transportation-details.component";
import {EWayBillsItemDetailsComponent} from "../../../generate-e-way-bills/screens/e-way-bills-item-details/e-way-bills-item-details.component";

@Component({
    selector: "app-generate-export-e-way-bills-form",
    templateUrl: "./generate-export-e-way-bills-form.component.html"
})
export class GenerateExportEWayBillsFormComponent implements OnInit {
    ...
    getInitialData() {
        this.spinner.show();
        this.salesInvoiceService.getAllEwayBillList({category: "Exports", type: "EwayBill"}).subscribe(success => {
    ...
    setSalesInvoice(eve: any) {
        this.submitted = true;
        this.spinner.show();
        this.salesInvoiceService.getById(eve._id).subscribe((success: any) => {
            let obj = {
                ...
                toGstin: "URP" || success?.customer?.GSTIN,
                toTrdName: success?.customer?.customerName,
                toAddr1: `${success?.customerBillingAddress?.line1} ${success?.customerBillingAddress?.line2}`,
                toAddr2: success?.customerBillingAddress?.line3,
                toPlace: success?.customerBillingAddress?.city,
                toPincode: 999999 || success?.customerBillingAddress?.pinCode,
                actToStateCode: 96 || success?.customer.GSTIN.slice(0, 2),
                toStateCode: 96 || success?.customer.GSTIN.slice(0, 2),
                transactionType: 1,
                otherValue: 0 || +(success?.otherCharges?.totalAmount).toFixed(2),
                totalValue: 0 || +(success?.salesInvoiceTotalTaxAmount).toFixed(2),
                cgstValue: 0 || +(success?.salesInvoiceTotalCGSTAmount).toFixed(2),
                sgstValue: 0 || +(success?.salesInvoiceTotalSGSTAmount).toFixed(2),
                igstValue: 0 || +(success?.salesInvoiceTotalIGSTAmount).toFixed(2),
    ...
    setStateCode() {
        this.f["toStateCode"].setValue(this.f["actToStateCode"].value);
    }
}
