...
import {QUOTATION_SKU_REPORT_DATA, QUOTATION_SKU_PDF_DATA} from "@mocks/export-data/sales/transactions";
import {QuotationOfSKU} from "@mocks/models/sales/transactions";
import {QuotationOfSKUService} from "@services/sales/quotationOfSKU.service";

@Component({
    selector: "app-quotation-of-sku-list",
    templateUrl: "./quotation-of-sku-list.component.html"
})
export class QuotationOfSkuListComponent implements OnInit, OnDestroy {
    ...
    update(u: any, action: string) {
        if (u?.status === "Approved") {
            this.spinner.show();
            this.quotationOfSKUService.update(u._id, {_id: u._id, status: action}).subscribe(success => {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.quotationOfSKUService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    pdfDownload(data: any) {
        let outPut = QUOTATION_SKU_PDF_DATA(data);
        this.exportToPDFService.generatePdf(outPut.tableData, outPut.title);
    }
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(QUOTATION_SKU_REPORT_DATA(data));
    }
