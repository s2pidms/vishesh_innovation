...
@Component({
    selector: "app-forecast-schedule-modal",
    templateUrl: "./forecast-schedule-modal.component.html",
    styles: [
        `
            .form-control-sch {
                margin: 0;
...
export class ForecastScheduleModalComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.bookSalesOrder == "Sales Forecast" && this.SOType == "Regular") {
    ...
    setQuantity(index: any) {
        ...
        if (totalQty > this.POQty) {
            if (this.bookSalesOrder == "Sales Forecast") {
                this.toastService.warning("total qty should not be greater than SO Qty");
            } else if (this.bookSalesOrder != "Sales Forecast") {
