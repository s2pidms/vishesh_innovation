...
import {
    PENDING_D_SKU_CONVERSION_PDF_DATA,
    PENDING_D_SKU_CONVERSION_REPORT_DATA
} from "@mocks/export-data/business-leads/reports";
import {NPDMasterService} from "@services/business-leads";
import {NgbdSortableHeader, SortEvent} from "@directives/sortable.directive";
import {PendingDSKUConversion} from "@mocks/models/business-leads/reports";

@Component({
    selector: "app-pending-d-sku-conversion",
    templateUrl: "./pending-d-sku-conversion.component.html"
})
export class PendingDSKUConversionComponent implements OnInit, OnDestroy {
    ...
    search: string = "";
    customerId: string = "";
    tableData: PendingDSKUConversion[] = [];
    statusOptions: any = [];
    customerOptions: any = [];
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.npdMasterService.getAllPendingDSKUConversionReport(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    pdfDownload(data: any) {
        let outPut = PENDING_D_SKU_CONVERSION_PDF_DATA(data);
        this.exportToPDFService.generatePdf(outPut.tableData, outPut.title);
    }
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(PENDING_D_SKU_CONVERSION_REPORT_DATA(data));
    }
