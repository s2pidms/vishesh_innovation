...
import {
    SAMPLE_JC_CREATION_REPORT_PDF_DATA,
    SAMPLE_JC_CREATION_REPORT_REPORT_DATA
} from "@mocks/export-data/business-leads/reports";

@Component({
    selector: "app-sample-jc-creation-report",
    templateUrl: "./sample-jc-creation-report.component.html"
})
export class SampleJcCreationReportComponent implements OnInit, OnDestroy {
    ...
    navigateTo(item: any, action: string) {
        ...
        window.open(
            `${window.location.origin}/#/print/sample_jc_creation_print?id=${item?._id}&action=${action}&buttonCondition=${buttonCondition}&orderType=${item?.orderType}`,
            "_blank"
        );
    }
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.sampleJCCreationService.getAllReports(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    pdfDownload(data: any) {
        let outPut = SAMPLE_JC_CREATION_REPORT_PDF_DATA(data);
        this.exportToPDFService.generatePdf(outPut.tableData, outPut.title);
    }
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(SAMPLE_JC_CREATION_REPORT_REPORT_DATA(data));
    }
