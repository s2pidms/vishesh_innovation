...
@Component({
    selector: "app-sample-jc-entry-list",
    templateUrl: "./sample-jc-entry-list.component.html"
})
export class SampleJcEntryListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.sampleJCEntryService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    update(item: any, action: string, id: string) {
        if (["Mark As Completed"].includes(item.status)) {
            this.spinner.show();
            this.sampleJCEntryService.update(id, {status: action}).subscribe(success => {
