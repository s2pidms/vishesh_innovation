...
import {SHIPMENT_PDF_DATA, SHIPMENT_REPORT_DATA} from "@mocks/export-data/dispatch/reports";
import {ShipmentReport} from "@mocks/models/dispatch/reports";
...
export class ShipmentReportComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        let payload = {
            ...
            toDate: this.toDate,
            customer: this.supplier
        };
        if (this.subscription) this.subscription.unsubscribe();
        this.subscription = this.shipmentPlanningService.getAllShipmentSummaryReports(payload).subscribe(success => {
            if (flag == "EXCEL") {
                ...
                this.supplierOptions = success.customers;
                this.collection = success.count;
                this.totalAmount = success?.totalAmounts?.shipmentValue;
            }
            this.spinner.hide();
        });
    }
    ...
    pdfDownload(data: any) {
        let output = SHIPMENT_PDF_DATA(data);
        this.exportToPDFService.generatePdf(output.tableData, output.title);
    }

    excelDownload(data: any) {
        this.exportExcelService.exportExcel(SHIPMENT_REPORT_DATA(data));
    }
