...
import {PURCHASE_ORDER_PDF_DATA, PURCHASE_ORDER_REPORT_DATA} from "@mocks/export-data/purchase/reports";
import {PurchaseOrder} from "@mocks/models/purchase/reports";
...
export class PurchaseOrderComponent implements OnInit, OnDestroy {
    ...
    navigateTo(u: any, action: string) {
        ...
        window.open(
            `${window.location.origin}/#/print/po_print?id=${u?._id}&action=${action}&buttonCondition=${buttonCondition}`,
            "_blank"
        );
    }
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.PurchaseService.getAllReports(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    pdfDownload(data: any) {
        let output = PURCHASE_ORDER_PDF_DATA(data);
        this.exportToPDFService.generatePdf(output.tableData, output.title);
    }

    excelDownload(data: any) {
        this.exportExcelService.exportExcel(PURCHASE_ORDER_REPORT_DATA(data));
    }
