...
import {PPV} from "@mocks/models/purchase/reports";

@Component({
    selector: "app-ppv",
    templateUrl: "./ppv.component.html"
})
export class PPVComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.purchaseService.getAllPpvReports(payload).subscribe(success => {
            if (flag == "EXCEL") {
                ...
                this.totalAmount = success?.totalAmounts?.ppvValue ? success?.totalAmounts?.ppvValue : 0;
            }
            this.spinner.hide();
        });
    }
    ...
    pdfDownload(data: any) {
        let output = PPV_PDF_DATA(data);
        this.exportToPDFService.generatePdf(output.tableData, output.title);
    }

    excelDownload(data: any) {
        this.exportExcelService.exportExcel(PPV_REPORT_DATA(data));
    }
