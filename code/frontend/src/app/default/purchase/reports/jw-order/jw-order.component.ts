...
import {JOB_WORK_ORDER_REPORT_PDF_DATA, JOB_WORK_ORDER_REPORT_REPORT_DATA} from "@mocks/export-data/purchase/reports";
import {JobWorkOrderService} from "@services/purchase/jobWorkOrder.service";

@Component({
    selector: "app-jw-order",
    templateUrl: "./jw-order.component.html"
})
export class JwOrderComponent implements OnInit, OnDestroy {
    ...
    navigateTo(u: any, action: string) {
        ...
        window.open(
            `${window.location.origin}/#/print/job_work_order_print?id=${u?._id}&action=${action}&buttonCondition=${buttonCondition}`,
            "_blank"
        );
    }
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.jobWorkOrderService.getAllReports(payload).subscribe(success => {
            if (flag == "EXCEL") {
                ...
                this.totalWOTaxableValue = success?.totalAmounts?.WOTaxableValue
                    ? (success?.totalAmounts?.WOTaxableValue).toFixed(2)
                    : 0;
                this.totalGSTAmount = success?.totalAmounts?.GSTAmount
                    ? (success?.totalAmounts?.GSTAmount).toFixed(2)
                    : 0;
                this.totalPOAmount = success?.totalAmounts?.totalAmountWithTax
    ...
    pdfDownload(data: any) {
        let output = JOB_WORK_ORDER_REPORT_PDF_DATA(data);
        this.exportToPDFService.generatePdf(output.tableData, output.title);
    }

    excelDownload(data: any) {
        this.exportExcelService.exportExcel(JOB_WORK_ORDER_REPORT_REPORT_DATA(data));
    }
