...
import {
    EXTERNAL_SERVICE_PROVIDER_MASTER_REPORT_DATA,
    EXTERNAL_SERVICE_PROVIDER_MASTER_PDF_DATA
} from "@mocks/export-data/purchase/master";
import {ExternalServiceProvider} from "@mocks/models/purchase/masters";
...
export class ExternalProviderListComponent implements OnInit, OnDestroy {
    ...
    direction: number = -1;
    search: string = "";
    tableData: ExternalServiceProvider[] = [];
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.externalServiceProviderService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    delete(id: any) {
        this.spinner.show();
        this.externalServiceProviderService.delete(id).subscribe(success => {
            this.spinner.hide();
    ...
    openConfirmModal(id: any, code: any) {
        ...
        modalRef.componentInstance.confirmText = `Confirm Deletion of ESP Code ${code} ?`;
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(EXTERNAL_SERVICE_PROVIDER_MASTER_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = EXTERNAL_SERVICE_PROVIDER_MASTER_PDF_DATA(data);
