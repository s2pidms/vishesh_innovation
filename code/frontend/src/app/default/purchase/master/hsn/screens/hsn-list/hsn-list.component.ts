...
import {HSN_MASTER_PDF_DATA, HSN_MASTER_REPORT_DATA} from "@mocks/export-data/purchase/master";
import {PurchaseHSN} from "@mocks/models/purchase/masters";
...
export class HSNListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.hsnService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    delete(id: any) {
        this.spinner.show();
        this.hsnService.delete(id).subscribe(success => {
            this.spinner.hide();
