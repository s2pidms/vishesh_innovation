...
import {DEBIT_NOTE_PDF_DATA, DEBIT_NOTE_REPORT_DATA} from "@mocks/export-data/purchase/transactions";
import {DebitNote} from "@mocks/models/purchase/transactions";
...
export class DebitListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.debitNoteService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    update(u: any, action: string) {
        if (u?.DNStatus === "Approved") {
            ...
            this.spinner.show();
            this.debitNoteService.update(u._id, u).subscribe(success => {
