...
import {PURCHASE_INDENT_REPORT_DATA, PURCHASE_INDENT_PDF_DATA} from "@mocks/export-data/purchase/transactions";
import {PurchaseIndentService} from "@services/purchase/purchaseIndent.service";
import {IPurchaseIndent} from "@mocks/models/purchase/transactions/purchaseIndent";

@Component({
    selector: "app-purchase-indent-list",
    templateUrl: "./purchase-indent-list.component.html"
})
export class PurchaseIndentListComponent implements OnInit, OnDestroy {
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.purchaseIndentService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    update(u: any, action: string) {
        if (u?.status === "Approved") {
            this.spinner.show();
            this.purchaseIndentService.update(u._id, {status: action}).subscribe(success => {
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(PURCHASE_INDENT_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = PURCHASE_INDENT_PDF_DATA(data);
