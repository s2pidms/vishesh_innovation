...
import {SALARY_COMPONENT_REPORT_DATA, SALARY_COMPONENT_PDF_DATA} from "@mocks/export-data/hr/master";
import {SalaryComponent} from "@mocks/models/hr&Admin/master";
...
export class ComponentsListComponent implements OnInit, OnDestroy {
    ...
    pageSize: number = 8;
    collection: number = 0;
    column: string = "earningHead";
    direction: number = 1;
    search: string = "";
    tableData: SalaryComponent[] = [];
    ...
    getAll(excel = false, flag = "") {
        ...
        this.subscription = this.SalaryComponentService.getAll(payload).subscribe(success => {
            if (flag == "EXCEL") {
    ...
    delete(id: any) {
        this.spinner.show();
        this.SalaryComponentService.delete(id).subscribe(success => {
            this.spinner.hide();
    ...
    openConfirmModal(id: any, code: any) {
        ...
        modalRef.componentInstance.confirmText = `Confirm Deletion of EarningHead ${code} ?`;
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(SALARY_COMPONENT_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = SALARY_COMPONENT_PDF_DATA(data);
