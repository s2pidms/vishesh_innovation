...
import {CANCEL_LEAVE_REPORT_DATA, CANCEL_LEAVE_PDF_DATA} from "@mocks/export-data/hr/transactions";
import {LeaveAdjustment} from "@mocks/models/hr&Admin/transactions";

@Component({
    selector: "app-cancel-leave-list",
    templateUrl: "./cancel-leave-list.component.html"
})
export class CancelLeaveListComponent implements OnInit, OnDestroy {
    ...
    navigateTo(path: string, u: any, action: string) {
        if (u.status == "Cancelled" && action == "cancelLeave") {
            return null;
        }
    ...
    excelDownload(data: any) {
        this.exportExcelService.exportExcel(CANCEL_LEAVE_REPORT_DATA(data));
    }

    pdfDownload(data: any) {
        let output = CANCEL_LEAVE_PDF_DATA(data);
